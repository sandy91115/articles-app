...
Route::get('/stories/{article:slug}', function (Article $article) {
    ...
    $description = e(Str::limit($article->preview_text, 180));
    $previewText = nl2br(e($article->preview_text), false);
    $author = e($article->author?->name ?? 'Mono Reader');
    $category = e($article->category);
    $price = number_format($article->price);
    $rating = number_format((float) $article->rating_average, 1);
    $views = number_format($article->view_count);
    $unlocks = number_format($article->unlock_count);
    $published = e(optional($article->published_at)->format('d M Y') ?? 'Recently');
    $currentUrl = e(url()->current());
    $homeUrl = e(url('/'));
    $imageUrl = $article->image_url ? e($article->image_url) : null;
    $imageMarkup = $imageUrl
...
<body>
    <main class="shell">
        <section class="hero">
            <div class="image-card">{$imageMarkup}</div>
            <article class="story-card">
                ...
                <div>
                    <a href="{$homeUrl}" class="cta">Open Reader Portal</a>
                    <p class="footer-note">
        ...
        <section class="body-grid">
            ...
            <aside class="stats">
                <div class="panel stat">Category<strong>{$category}</strong></div>
                <div class="panel stat">Views<strong>{$views}</strong></div>
                <div class="panel stat">Unlocks<strong>{$unlocks}</strong></div>
                <div class="panel stat">Published<strong>{$published}</strong></div>
            </aside>
        </section>
    </main>
</body>
